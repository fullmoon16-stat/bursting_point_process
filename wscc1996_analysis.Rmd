...
opt5 = optim(par=c(opt5$par[1],opt5$par[5],opt5$par[3]),fn5,control=list(fnscale=-1))
opt5 = optim(par=c(opt5$par[1],opt5$par[5],opt5$par[3]),fn5,control=list(fnscale=-1))
```
...
# actual_df: columns time, sequence (이미 맞다면)
actual_df <- tibble(
  time     = second,                # 실제 고장 시각 벡터
  sequence = seq_along(second)      # 누적 고장 수 또는 사건 인덱스
)
